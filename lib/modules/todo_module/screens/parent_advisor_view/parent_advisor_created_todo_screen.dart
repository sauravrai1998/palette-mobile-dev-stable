...
import 'package:palette/modules/todo_module/widget/parent_advisor_todo_widgets/parent_advisor_todo_list.dart';
...
class ParentAdvisorCreatedTodoScreen extends StatefulWidget {
  final BaseProfileUserModel? parentOrAdvisor;
  final bool isFromAdvisorDashboard;
  ParentAdvisorCreatedTodoScreen({
    Key? key,
    required this.parentOrAdvisor,
    this.isFromAdvisorDashboard = false,
  }) : super(key: key);

  @override
  _ParentAdvisorCreatedTodoScreenState createState() =>
      _ParentAdvisorCreatedTodoScreenState();
}

class _ParentAdvisorCreatedTodoScreenState
    extends State<ParentAdvisorCreatedTodoScreen> {
  ...
  Widget build(BuildContext context) {
    ...
    return BlocListener(
      listener: (context, parentTodoState) {
        if (parentTodoState is FetchTodoParentSuccessState) {
          // isFilterOn = false;
        }
      },
              ...
              child: Scaffold(
                body: Stack(children: [
                  Scaffold(
                      ...
                      child: !_searchSelected
                          ? AppBar(
                              ...
                              backgroundColor: _isCalendarViewSelected
                                  ...
                                  : Colors.transparent,
                              elevation: 0,
                              centerTitle: !_isCalendarViewSelected,
                    ...
                    body: widget.parentOrAdvisor is AdvisorProfileUserModel
                        ? Builder(builder: (context) {
                            if (_isCalendarViewSelected) {
                              return calendarView();
                            } else {
                              return taskListViewAdvisor(
                  ...
                  Visibility(
                    ...
                    child: GestureDetector(
                      onTap: () {
                        final parentOrAdvisor = widget.parentOrAdvisor;
                      ...
                      child: !_isCalendarViewSelected
                          ? Container(
                              child: Stack(children: [
                                ...
                                BlocBuilder<ProfileImageBloc,
                                        ProfileImageState>(
                                    builder: (context, state) {
                                  String? profilePictureUrl;
                                  final parentOrAdvisor =
                                      widget.parentOrAdvisor;
  ...
  Widget calendarView() {
    return NestedScrollView(
                  ...
                  flexibleSpace: FlexibleSpaceBar(
                    background: TodoCalendarWidget(
                      ...
                      onDaySelected: (datetime, dat) {
                        ...
                        // calendarTodoList.forEach((element) {
                        //   print(
                        //       '_focusedDay: $_focusedDay completeby: ${element.task.completeBy}');
                        // });

                        print(
                          'parent now the calendar list is: $filtersAppliedCalendarTodoList',
                        );
                      },
      ...
      body: BlocListener(
        listener: (context, state) {
          if (state is FetchTodoParentSuccessState) {
            calendarTodoList = state.todoParentListResponse.todoList;

            /*
print('_focusedDay: $_focusedDay: ${element.task.completeBy}');
            */
            calendarTodoList.forEach((element) {
              print('_focusedDay: $_focusedDay: ${element.task.completeBy}');
            });
          ...
          slivers: [
            ...
            BlocBuilder<TodoParentAdvisorBloc, TodoParentState>(
              builder: (context, state) {
                ...
                if (state is FetchTodoParentErrorState) {
                ...
                } else if (state is FetchTodoParentLoadingState) {
                  return SliverFillRemaining(
                    child: Center(
                      child: CustomPaletteLoader(),
                    ),
                  );
                } else if (state is FetchTodoParentSuccessState) {
                  // log("Entered Sliver Layout Builder");
                  print(
                      'filtersAppliedCalendarTodoList: ${filtersAppliedCalendarTodoList.length}');
                  ...
                  if (calendarTodoList.isEmpty) {
                    ...
                    return ParentAdvisorTodoListView(
                      useSliver: true,
        ...
        if (state is AdvisorStudentUserLoadingState) {
          ...
          return Column(
            children: [
              ...
              Expanded(
                child: ParentAdvisorTodoListTabView(
                    searchText: _searchText,
                    byme: byme,
                    sortDescending: sortDescending,
                    filterCheckboxHeadingList: filterCheckboxHeadingList,
                    parentOrAdvisor: widget.parentOrAdvisor!,
                    advisorStudents:
                        state.advisorStudentsMentorsList.data.students),
              ),
            ],
          );
  ...
  Widget taskListViewParent({
    ...
    return TextScaleFactorClamper(
      child: BlocBuilder<TodoParentAdvisorBloc, TodoParentState>(
        ...
        } else if (state is FetchTodoParentErrorState) {
  ...
  Widget _filterButton({required TodoFilterState todoFilterState}) {
    return BlocListener(
      bloc: context.read<TodoParentAdvisorBloc>(),
      listener: (context, state) {
        // print('ran listener in success');
        //
        // if (state is FetchTodoParentSuccessState) {
        //   listedByNames = state.todoParentListResponse.allListedByInOneList
        //       .map((e) => e.name)
        //       .toList();
        //
        ...
        //     byme: false,
        //
        //     /// byme false because we are in parent/advisor role
        //     // filterOptionsAssigned: true,
        //   ));
        //   filterOptionsAssigned = true;
        //   stateTodoList = state.todoParentListResponse.todoList;
        // }
        ...
        builder: (context, state) {
          if (state is FetchTodoParentSuccessState &&
            ...
            if (todoFilterState is TodoFilterListState &&
              ...
              final listedByName = state
                  .todoParentListResponse.allListedByInOneList
                  .map((e) => e.name)
                  .toList();
              filterCheckboxHeadingList =
                  getFilterCheckboxHeadingListForParentAndAdvisor(
                      listedByNames: listedByName);
            }

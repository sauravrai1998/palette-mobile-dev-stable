...
class _NotificationDetailScreenState extends State<NotificationDetailScreen> {
  ...
  ApproveNotificationDetailList? notification;
  bool isLoading = false;
  bool approvalSort = false;
  String? role;
  StreamController<List<ApproveNotificationDetail>> notificationStream =
      StreamController<List<ApproveNotificationDetail>>.broadcast();
  List<ApproveNotificationDetail> notificationList = [];
  ...
  Widget build(BuildContext context) {
    ...
    if (buildFirstTime) {
      NotificationPendoRepo.trackNotificationDetailVisit(
      ...
      child: Scaffold(
        body: LayoutBuilder(builder: (context, constraints) {
          return BlocBuilder<NotificationsDetailBloc, NotificationsDetailState>(
            ...
            if (state is FetchNotificationsDetailLoadingState) {
              print('loading');
              return _getLoadingIndicator();
            } else if (state is FetchNotificationsDetailSuccessState) {
              notification = state.notificationList;
              if(isFirstTime){
                 notification?.modelList?.forEach((element) {
                  if (element.type == 'Opportunity Approval Request') {
                    NotificationPendoRepo.trackViewApprovalOpportunity(
                        pendoState: pendostate);
                  }
                });
                isFirstTime = false;
              ...
              return StreamBuilder<List<ApproveNotificationDetail>>(
                  stream: notificationStream.stream,
                  builder: (context, snapshot) {
                    if (snapshot.data != null &&
                        snapshot.connectionState == ConnectionState.active) {
                      notificationList = snapshot.data!;
                      final type = notificationList[0].category ?? 'Other';
        ...
        children: [
          Column(
            ...
            children: [
              ...
              SizedBox(height: 15),
              if (notificationList[0].createdAt != null &&
                  notificationList[0].createdAt != 'null')
                Row(
                    mainAxisSize: MainAxisSize.min,
                    mainAxisAlignment: MainAxisAlignment.end,
                    children: [
                      ...
                      Text(
                        TimeAgo.timeAgoSinceDate(DateFormat('dd-MM-yyyy hh:mma')
                            .format(
                                DateTime.parse(notificationList[0].createdAt!)
                                    .toLocal())),
                        style: TextStyle(
                          fontSize: 14,
                          color: greyBorder,
              ...
              Container(
                ...
                // width: 150,
                child: Text(
                  '${notificationList[0].creatorName}',
                  style: TextStyle(
                    fontSize: 20,
          ...
          Align(
            ...
            child: Container(
                ...
                child: notificationList[0].creatorProfilePic != null &&
                        notificationList[0].creatorProfilePic != 'null'
                    ? CachedNetworkImage(
                        imageUrl: '${notificationList[0].creatorProfilePic}',
                        fit: BoxFit.cover,
                      )
                    : Container(
                        height: 50,
          ...
          children: [
            Column(
              ...
              children: [
                ...
                SizedBox(height: 20),
                Text('${notificationList[0].eventName}',
                    style: montserratBoldTextStyle.copyWith(
                        fontSize: 20, color: defaultDark)),
                SizedBox(height: 10),
                if (notificationList[0].phone != null) phoneRow(),
                SizedBox(height: 5),
                if (notificationList[0].venue != null) addressRow(),
                SizedBox(height: 5),
                if (notificationList[0].description != null) descContainer(),
              ],
            ),
            buttonRow(),
          ],
  ...
  Widget phoneRow() {
    return Visibility(
      visible: notificationList[0].phone!.isNotEmpty,
      child: Container(
        child: Row(
          children: [
            Icon(
  ...
  Widget addressRow() {
    return Visibility(
      visible: notificationList[0].venue!.isNotEmpty,
      child: Container(
        height: 30,
        child: Row(
    ...
    return Container(
      ...
      child: Scrollbar(
        child: SingleChildScrollView(
          child: Text('${notificationList[0].description}',
      ...
      child: Row(
        ...
        children: [
          openButton(onPressed: () {
            // _launchInBrowser(notificationList[0].);
          }),
          approveButton(
              onPressed: notificationList[0].approvalStatus == 'Approved' ||
                      (role!.toLowerCase() == 'advisor' &&
                          notificationList[0].approvalStatus == 'In Review')
                  // notificationList[0].approvalStatus == 'AdvisorReview'
                  ? () {}
                  : () async {
                      ///
                      final pendoState =
                      ...
                      bool result = await NotificationsRepository.instance
                          .approveDismissRequest(
                              status: 'Accept',
                              eventId: notificationList[0].id!,
                              notificationId: widget.notificationItemModel.id,
                              notificationType:
                                  widget.notificationItemModel.type);
                      if (!result) {
                        // NotificationsRepository.instance.approveDismissRequest(
                        //     status: 'Reject',
                        //     eventId: notificationList[0].id!,
                        //     notificationId: widget.notificationItemModel.id,
                        //     notificationType:
          ...
          cancelButton(
              onPressed: notificationList[0].approvalStatus == 'Rejected'
                  ? () {}
                  : () async {
                final pendoState =
                    BlocProvider.of<PendoMetaDataBloc>(context).state;
                ...
                bool result = await NotificationsRepository.instance
                    ...
                    eventId: notificationList[0].id!,
                    notificationId:
                    widget.notificationItemModel.id,
                    notificationType:
                    widget.notificationItemModel.type);
                widget.removeApprovalCallBack(
                    widget.notificationItemModel);
  ...
  Widget openButton({required Function onPressed}) {
    return Visibility(
      visible: notificationList[0].website != null &&
          notificationList[0].website != 'null',
  ...
  Widget approveButton({required Function onPressed}) {
    return Visibility(
      visible: notificationList[0].approvalStatus != 'Rejected' &&
          // notificationList[0].valid! &&
          notificationList[0].approvalStatus != 'null',
      child: GestureDetector(
        onTap: () => onPressed(),
        child: isLoading
            ? Container(
            ...
            : Row(children: [
                Text(
                    notificationList[0].approvalStatus == 'Approved' ||
                            // notificationList[0].approvalStatus ==
                            //     'AdvisorReview'
                            (role!.toLowerCase() == 'advisor' &&
                                notificationList[0].approvalStatus ==
                                    'In Review')
  ...
  Widget cancelButton({required Function onPressed}) {
    return Visibility(
      visible: notificationList[0].approvalStatus != 'Approved' &&
          // notificationList[0].approvalStatus != 'AdvisorReview' &&
          // notificationList[0].valid! &&
          notificationList[0].approvalStatus != 'null',
      child: (role!.toLowerCase() == 'advisor' &&
              notificationList[0].approvalStatus == 'In Review')
          ? Container()
          : GestureDetector(
              onTap: () => onPressed(),
              child: isDismiss
                  ...
                  : Row(
                      children: [
                        Text(
                            notificationList[0].approvalStatus == 'Rejected'
                                ? 'REJECTED'
                                : '',
                            style: robotoTextStyle.copyWith(
                                fontSize: 16,
